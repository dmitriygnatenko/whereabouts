...
const TRANSLATIONS = {
  ru: {
    ...
    'Location updated': 'Место обновлено',
...
const SERVER_ERRORS = {
  ru: {
    ...
    'Failed to generate a token': 'Не удалось сгенерировать токен',
    ...
    'Location filter depth must be 0 (show all) or at least 1': 'Глубина вложенности мест в фильтрах должна быть 0 (показывать все) или не менее 1',
    'Failed to save location filter depth': 'Не удалось сохранить глубину вложенности мест в фильтрах',
    ...
    'Enter the item name': 'Введите название вещи',
    'Item name must be at most 255 characters': 'Название вещи должно быть не длиннее 255 символов',
    'Choose a location': 'Выберите место хранения',
    ...
    'Location name must be at most 255 characters': 'Название места должно быть не длиннее 255 символов',
  ...
  de: {
    ...
    'Failed to generate a token': 'Token konnte nicht generiert werden',
    ...
    'Location filter depth must be 0 (show all) or at least 1': 'Die Verschachtelungstiefe der Orte in Filtern muss 0 (alle anzeigen) oder mindestens 1 sein',
    'Failed to save location filter depth': 'Verschachtelungstiefe der Orte in Filtern konnte nicht gespeichert werden',
    ...
    'Enter the item name': 'Geben Sie den Namen des Gegenstands ein',
    'Item name must be at most 255 characters': 'Der Name des Gegenstands darf höchstens 255 Zeichen lang sein',
    'Choose a location': 'Wählen Sie einen Ort',
    ...
    'Location name must be at most 255 characters': 'Der Name des Ortes darf höchstens 255 Zeichen lang sein',
  ...
  es: {
    ...
    'Failed to generate a token': 'No se pudo generar el token',
    ...
    'Location filter depth must be 0 (show all) or at least 1': 'La profundidad de anidación de ubicaciones en filtros debe ser 0 (mostrar todos) o al menos 1',
    'Failed to save location filter depth': 'No se pudo guardar la profundidad de anidación de ubicaciones en filtros',
    ...
    'Enter the item name': 'Introduce el nombre del objeto',
    'Item name must be at most 255 characters': 'El nombre del objeto debe tener como máximo 255 caracteres',
    'Choose a location': 'Elige una ubicación',
    ...
    'Location name must be at most 255 characters': 'El nombre de la ubicación debe tener como máximo 255 caracteres',
  ...
  fr: {
    ...
    'Please enter a username': "Saisissez un nom d'utilisateur",
    'Username must be at least 3 characters': "Le nom d'utilisateur doit comporter au moins 3 caractères",
    'Please enter a password': 'Saisissez un mot de passe',
    ...
    'Failed to generate a token': 'Impossible de générer un jeton',
    ...
    'Location filter depth must be 0 (show all) or at least 1': "La profondeur d'imbrication des emplacements dans les filtres doit être 0 (tout afficher) ou au moins 1",
    'Failed to save location filter depth': "Impossible d'enregistrer la profondeur d'imbrication des emplacements dans les filtres",
    ...
    'Enter the item name': "Saisissez le nom de l'objet",
    'Item name must be at most 255 characters': "Le nom de l'objet ne doit pas dépasser 255 caractères",
    'Choose a location': 'Choisissez un emplacement',
    ...
    'Location name must be at most 255 characters': "Le nom de l'emplacement ne doit pas dépasser 255 caractères",
...
const SERVER_ERROR_PATTERNS = {
  ru: [
    ...
    { re: /^photo #(\d+): failed to save \((.+)\)$/,
      fmt: (m, locale) => `Фото №${m[1]}: не удалось сохранить (${localizeServerMessage(locale, m[2])})` },
    // Fallback for any other "photo #N: <reason>" shape (e.g. a malformed data URL) — translates
    // the reason by exact match if known, otherwise leaves it as the original English text.
    { re: /^photo #(\d+): (.+)$/,
      fmt: (m, locale) => `Фото №${m[1]}: ${localizeServerMessage(locale, m[2])}` },
  ],
  de: [
    { re: /^Invalid request body: request too large \(max (\d+) MB\)$/,
    ...
    { re: /^photo #(\d+): failed to save \((.+)\)$/,
      fmt: (m, locale) => `Foto Nr. ${m[1]}: konnte nicht gespeichert werden (${localizeServerMessage(locale, m[2])})` },
    { re: /^photo #(\d+): (.+)$/,
      fmt: (m, locale) => `Foto Nr. ${m[1]}: ${localizeServerMessage(locale, m[2])}` },
  ],
  es: [
    { re: /^Invalid request body: request too large \(max (\d+) MB\)$/,
    ...
    { re: /^photo #(\d+): failed to save \((.+)\)$/,
      fmt: (m, locale) => `Foto n.º ${m[1]}: no se pudo guardar (${localizeServerMessage(locale, m[2])})` },
    { re: /^photo #(\d+): (.+)$/,
      fmt: (m, locale) => `Foto n.º ${m[1]}: ${localizeServerMessage(locale, m[2])}` },
  ],
  fr: [
    { re: /^Invalid request body: request too large \(max (\d+) MB\)$/,
    ...
    { re: /^photo #(\d+): failed to save \((.+)\)$/,
      fmt: (m, locale) => `Photo n° ${m[1]} : échec de l'enregistrement (${localizeServerMessage(locale, m[2])})` },
    { re: /^photo #(\d+): (.+)$/,
      fmt: (m, locale) => `Photo n° ${m[1]} : ${localizeServerMessage(locale, m[2])}` },
  ],
};
