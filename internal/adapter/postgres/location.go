...
func (s *Storage) FindLocationParentID(ctx context.Context, id uint64) (*uint64, error) {
	...
	err := s.DB.QueryRowContext(ctx, `SELECT parent_id FROM locations WHERE id = $1`, id).Scan(&parentID)
	if err != nil {
		return nil, err
	}
...
func (s *Storage) CreateLocation(
	...
	return s.insertReturningID(ctx,
		`INSERT INTO locations (title, color, parent_id, created_at) VALUES ($1, $2, $3, $4) RETURNING id`,
...
func (s *Storage) UpdateLocation(ctx context.Context, id uint64, name, color string) (bool, error) {
	res, err := s.DB.ExecContext(ctx,
		`UPDATE locations SET title = $1, color = $2 WHERE id = $3`, name, color, id,
	)
	if err != nil {
		return false, err
	}
...
func (s *Storage) CountLocationChildren(ctx context.Context, parentID uint64) (int, error) {
	var n int
	err := s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM locations WHERE parent_id = $1`, parentID).Scan(&n)
