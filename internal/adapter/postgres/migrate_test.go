...
// CREATE INDEX statements the schema is built from, in order — Postgres, like SQLite and unlike
// MySQL, can't declare an index inline inside CREATE TABLE, so each one is its own statement. Without
// a real server to inspect afterward, this is what "creates the schema" can mean under a mock: it
// can't confirm the SQL is valid or that the resulting tables behave as declared — that's what an
// integration test against a real Postgres server would be for — only that the adapter attempted to
