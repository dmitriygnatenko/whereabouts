...
func TestCreateSession(t *testing.T) {
	...
	query := `INSERT INTO sessions (token, user_id, created_at, expires_at) VALUES ($1, $2, $3, $4)`
	...
	tests := []struct {
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, createdAt, expiresAt).WillReturnError(pgErr(pgForeignKeyViolation))
			},
			assertErr: func(t *testing.T, err error) { require.Error(t, err) },
		},
		{
			name: "a token already in use surfaces the driver's unique violation error",
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, createdAt, expiresAt).WillReturnError(pgErr(pgUniqueViolation))
			},
			assertErr: func(t *testing.T, err error) { require.Error(t, err) },
		},
	}
...
// (WHERE expires_at < $1) is the part that belongs to this adapter.
