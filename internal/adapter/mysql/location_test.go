...
func TestCreateLocation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			parent: &parentID,
			mock: func(mock sqlmock.Sqlmock, parent any) {
				mock.ExpectExec(query).WithArgs(name, color, parent, createdAt).WillReturnError(mysqlErr(mysqlNoReferencedRow))
			},
			wantErr: true,
		},
	}
...
func TestDeleteLocation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "an unknown id is reported as not found", res: sqlmock.NewResult(0, 0)},
		{name: "a location still holding rows surfaces the driver's constraint error", mockErr: mysqlErr(mysqlNoReferencedRow), wantErr: true},
	}
