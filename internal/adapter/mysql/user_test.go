...
//
// Not covered here: MySQL's utf8mb4_unicode_ci collation on this column makes the real lookup (and
// its UNIQUE constraint, see TestCreateUser) case-insensitive, unlike SQLite's binary collation. That
// behavior belongs to the server, not this adapter's Go code, so a mock — which just returns whatever
// row it's told to — can't exercise it; it'd need an integration test against a real server.
...
func TestCreateUser(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(username, hash, mustJSON(settings)).
					WillReturnError(mysqlErr(mysqlDuplicateEntry))
			},
			wantUnique: true,
		},
	}
...
func TestUpdateUsername(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a taken username is a unique violation", mockErr: mysqlErr(mysqlDuplicateEntry), wantUnique: true},
	}
...
// than clobbering it is MySQL's own behavior — verifying the merge itself needs a real server, out of
// reach for a mock that only ever reports "the statement ran".
