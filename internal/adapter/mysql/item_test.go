...
func TestListItemImages(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// MySQL rejects "IN ()" outright as a syntax error rather than treating it as an empty
			// match. That's exactly why the interface this implements documents itemIDs as "must not
			// be empty" (internal/repository/item/repository.go), and its only caller (imagesFor)
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(placeholdersQuery(0)).WillReturnError(mysqlErr(1064)) // ER_PARSE_ERROR
			},
			assertResult: func(t *testing.T, got map[uint64][]model.ItemImage) {},
...
func TestCreateItem(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(name, locationID, notes, now, now).
					WillReturnError(mysqlErr(mysqlNoReferencedRow))
			},
			assertResult: func(t *testing.T, got uint64) {},
			assertErr:    func(t *testing.T, err error) { require.Error(t, err) },
		},
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock, images []model.ItemImage) {
				...
				mock.ExpectExec(`DELETE FROM item_images WHERE item_id = ?`).
					WithArgs(itemID).WillReturnResult(sqlmock.NewResult(0, 0))
				mock.ExpectExec(`INSERT INTO item_images (item_id, url, thumbnail_url, position) VALUES (?, ?, ?, ?)`).
					WithArgs(itemID, images[0].URL, images[0].ThumbnailURL, 0).WillReturnError(mysqlErr(mysqlNoReferencedRow))
				mock.ExpectRollback()
			},
			assertErr: func(t *testing.T, err error) { require.Error(t, err) },
		},
