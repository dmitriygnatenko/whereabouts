...
func (s *Storage) ListItemImages(
	...
	// #nosec G202 -- the interpolated text is a run of "?" placeholders built from len(itemIDs); the ids
...
func (s *Storage) ItemImages(ctx context.Context, itemID uint64) ([]model.ItemImage, error) {
	rows, err := s.DB.QueryContext(ctx,
		`SELECT url, thumbnail_url FROM item_images WHERE item_id = ? ORDER BY position`, itemID,
	)
	if err != nil {
		return nil, err
	}
...
func (s *Storage) ReplaceItemImages(ctx context.Context, itemID uint64, images []model.ItemImage) error {
	...
	for position, img := range images {
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO item_images (item_id, url, thumbnail_url, position) VALUES (?, ?, ?, ?)`,
...
func (s *Storage) CountItemsByLocation(ctx context.Context, locationID uint64) (int, error) {
	var n int
	err := s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM items WHERE location_id = ?`, locationID).Scan(&n)
