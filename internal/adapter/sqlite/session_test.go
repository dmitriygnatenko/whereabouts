...
// errors rather than enforced by a real database file.
func TestCreateSession(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, createdAt, expiresAt).WillReturnError(sqliteForeignKeyErr())
			},
			wantErr: true,
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, createdAt, expiresAt).WillReturnError(sqliteUniqueErr())
			},
			wantErr: true,
		},
	}
...
// schema/driver's job to get right, not something a mock can verify — the query text this pins down
