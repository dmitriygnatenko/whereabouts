...
func TestTimeScan(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			in:   withNanos.Format(time.RFC3339Nano),
			want: withNanos,
		},
		{
			name: "RFC3339 string",
		...
		},
		{
			...
			in:   want.Format("2006-01-02 15:04:05"),
			want: want,
		},
		{
			name: "offset string",
		...
		{
			name: "[]byte",
			in:   []byte(want.Format(time.RFC3339)),
			want: want,
		},
		{
			name: "nil column",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.True(t, got.Equal(tt.want), "Scan(%v) = %v, want %v", tt.in, got.Time, tt.want)
		})
	}
}
